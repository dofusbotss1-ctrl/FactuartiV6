...
export default function StockManagement() {
 ...
 {/* ========= Rapport (caché) — donuts compact + header pro ========= */}
<div
  ref={reportRef}
  style={{
    display: 'none',
...
>
  {/* Styles spéciaux pour le PDF (évitent les débordements) */}
  <style>{`
    .pdf-reset * { box-shadow: none !important; filter: none !important; }
    .pdf-reset img, .pdf-reset canvas, .pdf-reset svg { max-width: 100% !important; height: auto !important; }
    .pdf-card { border:1px solid #E5E7EB; border-radius:8px; background:#fff; padding:10px; overflow:hidden; }
    .pdf-chart { width: 210px; height: 210px; margin: 0 auto; display:block; overflow:hidden; }
    .pdf-grid-2 { display:grid; grid-template-columns: 1fr 1fr; gap:10px; }
    .pdf-h { color:#111827; font-weight:800; }
    .pdf-sub { color:#475569; }
    .pdf-meta { font-size:10px; color:#475569; text-align:right; }
    .pdf-title { font-size:18px; color:#4F46E5; font-weight:900; letter-spacing:.2px; }
    .pdf-table th, .pdf-table td { border:1px solid #E5E7EB; padding:6px; }
    .pdf-table th { background:#F1F5F9; color:#334155; font-weight:800; }
    .pdf-zebra tr:nth-child(even) { background:#F8FAFC; }
  `}</style>
  ...
  {/* ===== KPIs (inchangé si tu veux) ===== */}
  <section className="pdf-reset" style={{ padding: '10px 14px 6px' }}>
    <div className="pdf-card" style={{ background:'linear-gradient(90deg, rgba(99,102,241,0.06), rgba(16,185,129,0.06))' }}>
      <div className="pdf-h" style={{ fontSize:12, marginBottom:6 }}>Statistiques globales</div>
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:10.5 }}>
        <tbody>
          <tr><td className="pdf-table" style={{ width:'50%' }}>Stock initial</td><td className="pdf-table" style={{ fontWeight:700 }}>{stats.totalStockInitial.toFixed(0)} {unitLabel()}</td></tr>
          <tr><td className="pdf-table">Valeur d'achat</td><td className="pdf-table" style={{ fontWeight:700 }}>{stats.totalPurchaseValue.toLocaleString()} MAD</td></tr>
          <tr><td className="pdf-table">Valeur de vente</td><td className="pdf-table" style={{ fontWeight:700 }}>{stats.totalSalesValue.toLocaleString()} MAD</td></tr>
          <tr>
            <td className="pdf-table">Marge brute</td>
            <td className="pdf-table" style={{ fontWeight:800, color: stats.grossMargin >= 0 ? '#059669' : '#DC2626' }}>
              {stats.grossMargin >= 0 ? '+' : ''}{stats.grossMargin.toLocaleString()} MAD
            </td>
          </tr>
          <tr><td className="pdf-table">Stock vendu</td><td className="pdf-table" style={{ fontWeight:700 }}>{stats.totalQuantitySold.toFixed(0)} {unitLabel()}</td></tr>
          <tr>
            <td className="pdf-table">Stock rectif</td>
            <td className="pdf-table" style={{ fontWeight:800, color: stats.totalRectif >= 0 ? '#2563EB' : '#DC2626' }}>
              {stats.totalRectif >= 0 ? '+' : ''}{stats.totalRectif.toFixed(0)} {unitLabel()}
            </td>
          </tr>
          <tr><td className="pdf-table">Stock restant</td><td className="pdf-table" style={{ fontWeight:800 }}>{stats.totalRemainingStock.toFixed(0)} {unitLabel()}</td></tr>
        </tbody>
      </table>
    </div>
  </section>
  ...
  {/* ===== Donuts compacts ===== */}
  <section className="pdf-reset" style={{ padding: '6px 14px' }}>
    <div className="pdf-grid-2">
      <div className="pdf-card">
        <div className="pdf-h" style={{ fontSize:11, marginBottom:6 }}>Répartition des ventes</div>
      ...
      <div className="pdf-card">
        <div className="pdf-h" style={{ fontSize:11, marginBottom:6 }}>Valeur du stock restant</div>
        <div className="pdf-chart">
          <DonutChart
            data={stockDonutData}
            title=""
  ...
  {/* ===== Tableau détaillé (zébré) ===== */}
  <section className="pdf-reset" style={{ padding: '8px 14px 10px' }}>
    <div className="pdf-h" style={{ fontSize:12, marginBottom:8 }}>Analyse détaillée par produit</div>
    <table className="pdf-table pdf-zebra" style={{ width:'100%', borderCollapse:'collapse', fontSize:10.5 }}>
      <thead>
        <tr>
          {['Produit','Stock initial','Qté vendue','Stock rectif','Stock restant','Achat (MAD)','Vente (MAD)','Marge (MAD)'].map((h,i)=>(
      ...
      <tbody>
        {getDetailedProductData().map((p)=>(
          <tr key={p.id}>
            <td>{p.name}</td>
            <td style={{ textAlign:'right' }}>{p.stock.toFixed(3)} {p.unit||''}</td>
            <td style={{ textAlign:'right' }}>{p.quantitySold.toFixed(3)} {p.unit||''}</td>
            <td style={{ textAlign:'right', color: p.rectif>=0?'#2563EB':'#DC2626', fontWeight:700 }}>
              {p.rectif>=0?'+':''}{p.rectif.toFixed(3)} {p.unit||''}
            </td>
            <td style={{ textAlign:'right' }}>{p.remainingStock.toFixed(3)} {p.unit||''}</td>
            <td style={{ textAlign:'right' }}>{p.purchaseValue.toLocaleString()}</td>
            <td style={{ textAlign:'right' }}>{p.salesValue.toLocaleString()}</td>
            <td style={{ textAlign:'right', color: p.margin>=0?'#059669':'#DC2626', fontWeight:800 }}>
  ...
  {/* ===== Ventes mensuelles compact ===== */}
  <section className="pdf-reset" style={{ padding: '6px 14px 12px' }}>
    <div className="pdf-card">
      <div className="pdf-h" style={{ fontSize:11, marginBottom:6 }}>Ventes mensuelles {selectedYear}</div>
      <div style={{ width: 560, height: 300, margin:'0 auto', overflow:'hidden' }}>
  ...
  {/* Footer */}
  <section className="pdf-reset" style={{ padding: 12, borderTop:'1px solid #E5E7EB', textAlign:'center' }}>
